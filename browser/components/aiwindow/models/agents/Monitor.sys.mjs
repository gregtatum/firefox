...
// TODO: Move these constants to RS
...
export const MONITOR_PROMPT_VERSION = String(
  FEATURE_MAJOR_VERSIONS[MODEL_FEATURES.CHAT] // TODO: make this MODEL_FEATURES.AGENT_MONITOR once in RS
);
...
export class Monitor {
  ...
  constructor({
    ...
    this.watchUrls = normalizeWatchUrls(watchUrls);
  ...
  async runMonitorCheck({
    ...
    const conversation = await lazy.buildConversation(
      MODEL_FEATURES.CHAT,
      // MODEL_FEATURES.AGENT_MONITOR, // TODO: once in RS
      { flowId }
    );
    ...
    // render the system and user prompt with the monitor request, page URL, check time, and page content
    const systemPrompt = renderPrompt(MONITOR_AGENT_SYSTEM_PROMPT, {
      monitorPrompt: this.monitorPrompt,
    });
    const userPrompt = renderPrompt(MONITOR_AGENT_USER_PROMPT, {
...
function normalizeLoadedHistory(history) {
  ...
  return history.map(entry => {
    ...
    return {
      ...entry,
      status: "error",
      result: "Monitor check was interrupted before it finished.",
      conditionMet: false,
    };
  });
}
...
export function normalizeWatchUrls(urls) {
  return (Array.isArray(urls) ? urls : [])
    ...
    .filter(isAllowedWatchUrl)
    .slice(0, TOTAL_NUM_URLS_IN_MONITOR);
}
