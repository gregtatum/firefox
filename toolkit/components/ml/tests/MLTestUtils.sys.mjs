...
export class MockLLMEngine {
  ...
  /**
   * Run the inference request with a mocked response. If the mocked response is a string
  ...
  async run(request) {
    ...
    if (Array.isArray(response)) {
      return { finalOutput: response.join(""), ...EXTRA_RESPONSE_ARGS };
    }
...
export const MLTestUtils = {
  ...
  serveHTML(options = {}) {
    ...
    function html(strings, ...values) {
      return createServer(String.raw(strings, ...values), code);
    }
   ...
   *
   * Example usage:
   *   const { html } = setupHTMLServer({ browser: gBrowser, code: 404 });
   *   const { cleanup, tab } = await html`
  ...
  serveHTMLInTab(options) {
    ...
    async function html(strings, ...values) {
      const markup = `<!DOCTYPE html><body>${String.raw(strings, ...values)}</body>`;
