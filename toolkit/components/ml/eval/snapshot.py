...
class Snapshot:
    def __init__(self, command_context, headless: bool):
        self.marionette = Snapshot.setup_marionette(command_context, headless)

    def run(self):
        try:
            self.marionette.navigate("about:blank")
            with self.marionette.using_context("chrome"):
                self.marionette.execute_script("dump('Chrome context');")
    ...
    @staticmethod
    def setup_marionette(command_context, headless: bool) -> Any:
        command_context.activate_virtualenv()
        eval_tools_dir = Path(command_context.topobjdir) / "eval-tools"
        eval_tools_dir.mkdir(parents=True, exist_ok=True)

        addon_path = Snapshot.setup_singlefile_addon(eval_tools_dir)
