...
class _Evaluation(ABC):
    ...
    def __init__(
        ...
        self.log = log
        # Should this alert in perfherder?
        self.shouldAlert = shouldAlert
        self.alertThreshold = alertThreshold

    @abstractmethod
    def run(self, test_name: str, payloads: list[dict[str, Any]]) -> dict:
...
class _LlmJudge(_Evaluation):
    ...
    def __init__(
        ...
        alertThreshold: Optional[float] = None,
        token: str | None = None,
        endpoint: str = "https://mlpa-nonprod-stage-mozilla.global.ssl.fastly.net/v1/chat/completions",
        model: str = "vertex_ai/mistral-small-2503",
        **kwargs,
    ) -> None:
        super().__init__(log, shouldAlert, alertThreshold)
        self.endpoint = endpoint
        self.model = model
        self.token = token or os.environ.get("MOZ_FXA_BEARER_TOKEN")
...
class _TranslationsSacreBleu(_Evaluation):
    ...
    def run(self, test_name: str, payloads: list[dict[str, Any]], log=None):
        ...
        return {
            ...
            "subtest": test_name,
            "data": [
                {"file": "eval-mochitest", "value": value, "xaxis": index}
                for index, value in enumerate(results)
            ],
            "value": sum(results) / len(results),
            "unit": self.name,
            "shouldAlert": self.shouldAlert,
...
class TranslationsLlmJudge(_LlmJudge):
    ...
    def run(self, test_name: str, payloads: list[dict[str, Any]]):
        ...
        return {
            ...
            "value": sum(result.get("score", 0) for result in results) / len(results),
            "unit": "score",
            "shouldAlert": self.shouldAlert,
            "alertThreshold": self.alertThreshold,
            "lowerIsBetter": False,
