...
async function createTranslationsDoc(html, options) {
  ...
  async function htmlMatches(
    ...
    let simulatedIntersectionObservation = false;
    ...
    try {
      await waitForCondition(async () => {
        ...
        if (!htmlMatches && !simulatedIntersectionObservation) {
          // If this is a subsequent attempt with no match, and no new resolved promises,
          // then it may be because the request was never sent to the scheduler, so we need
          // to manually simulate intersection observation.
          //
          ...
          simulatedIntersectionObservation = true;
          translationsDoc.simulateIntersectionObservationForNonPendingNodes();
        }
...
async function closeFindBar(tab) {
