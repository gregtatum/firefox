...
export class TranslationsDocument {
  ...
  #markNodeContentMutated(node) {
    ...
    const selfOrParentElement =
      asElement(node) ?? asElement(node.parentElement);
  ...
  #maybeMarkElementAttributeMutated(element, attributeName, oldValue) {
    ...
    if (newValue.length === 0) {
      // The new attribute is empty, so there is nothing to translate.
      return;
    }
   ...
   * Nodes that are added to the queue will be prioritized and sent to the scheduler.
   *
   * @param {Element} element
   */
